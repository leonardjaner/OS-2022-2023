...
int main()
{
	...
	if ((file_read=open("./destination_file.txt",O_RDONLY)) < 0) err_sys("error with destination (source) file");
	if ((input=open("./source_file.txt",O_RDONLY)) < 0) err_sys("error with source file");
